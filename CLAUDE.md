...
1. **任何开发任务 / 缺陷修复 / 算法实验，必须走 `MLVR_develop/README.md` 的四步流程**：
   立项 → ⛔ 人拍板 → 实施+自验 → 归档。未经拍板不改 RMC；验证贴真实输出；改动留 `changes.diff`。
   用 `MLVR_develop/new_task.sh <任务短名> [KB编号]` 建档，原始材料（报错/数据/日志）
   **原样**存进任务文件夹的 `logs/`。
   **交互要求**：Agent 在每个阶段结束或遇到影响方向的岔路时主动向人提问并等待，不把问题写成文档让人自己读。
   **验证深度**：够用就停——能支撑下一步决策即通过；RMC 为成熟软件，不重复验证前人已完成的功能。
