...
1. **任何开发任务 / 缺陷修复 / 算法实验，必须先按 `MLVR_develop/README.md` 选择 A/B/C 任务模式，再走五步流程**：
   立项 → 设计 → ⛔ 人拍板 → 实施+自验 → 归档。默认 B；涉及物理结论、适用边界、统计解释或训练数据治理时建议 C。
...
10. **验证分场景，不做无限验证**：物理结论从严（可复现命令 + 输出 + 必要统计量）；工程/文档/治理从简（跑通代表性用例即可）。
    默认信任前人实现，审查以抽查关键路径为主；说不清的写"未验证"，不要堆验证。
11. **随时问、宁多勿少**：需求不清、方案取舍、物理判断、范围边界直接问用户，每次 ≤3 个问题并尽量给可选项；
    不用长文档代替对话——档案是给人复核的，不是给人阅读的。
12. **上下文纪律（省 token、防幻觉）**：开场只读 `STATUS.md` + `MLVR_Knowledge/AGENT_CONTEXT.md`；长文档先读头部“导航”再按行区间取节；
    要事实用 `grep -n` 定位，**不要通读知识库或历史档案**。结论类事实只在档案 §4 与 `02 审查矩阵`顶部速览维护，其它文档只写指针、不复述。
