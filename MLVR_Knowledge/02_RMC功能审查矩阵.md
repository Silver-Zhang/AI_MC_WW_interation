...
Stage 2 将依据本矩阵逐项进行只读审查；当前文件只定义审查对象，不预判 RMC 实现状态。
...
| ID | 功能需求 | RMC状态 | 审查重点/备注 |
|---|---|---|---|
| F01 | Forward fixed-source MC | 待审查 | Bootstrap 与正式 Forward iteration 的基础能力 |
| F02 | 多群 Adjoint transport | 待审查 | 重点核查实际物理实现与可用状态 |
...
对 RMC 中不常用、理论上需要特别确认或组合后可能失效的功能（尤其多群 Adjoint、Adjoint + WW、Adjoint field tally），应提供更强的源码与数值证据。
