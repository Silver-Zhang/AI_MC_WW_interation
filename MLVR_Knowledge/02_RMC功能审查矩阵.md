...
| F02-B | 多群Adjoint物理正确性审查 | Stage 3 修复中（E — Defect） | W5 数值确认且未修复；W6 动态频数未覆盖且未修复；W7 已修复验证；V4 非裂变代表子域互易性通过 | `20260824_f02-adjoint-physics-verification`；`20260825_f02-adjoint-numerical-verification`；`20260825_f02-w7-neutron-only-adjoint-init-fix` |
...
对局部密度比例恒为 $r=1$ 且所有裂变核素 `NNUBAR<=1` 的受限子域，当前结论仍为 **C — Verify**。第一阶段 L4 已补齐当前 SHA 的既有回归，并在 H2O 均匀球中验证两个强非对称散射群对；但这只是非裂变 P0 代表案例，尚缺混合材料碰撞估计器、一般几何/源响应、强各向异性方向互易性和可裂变 bank/权重数值验证。W7 已获用户授权进入 Stage 3 并完成修复；W5/W6 未经各自修复任务拍板前仍不修改，也不进入 F03。
...
- 2026-08-25：完成 W7 Stage 3 修复与验证；纯中子伴随初始化不再访问 photon 群，原崩溃输入正常完成。W5/W6 未修复，完整能力保持 E。
